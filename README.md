...
### アクセス方法（VSCode ポートフォワーディング）

このコンテナはポート 8888 をホストに自動公開していないため、**VSCode のポートフォワーディング**を使う必要があります。

1. VSCode 下部の「ポート」タブを開く
2. 「+」ボタンをクリックし、`8888` を入力してポートを転送
3. ターミナルに表示されたトークン付き URL をそのままブラウザで開く（**`http://`** であること。`https://` は不可）:
   ```
   http://127.0.0.1:8888/lab?token=xxxxxxxx...
   ```
   または `http://localhost:8888` を開いて、表示されるログイン画面の「Password or token」欄にトークン文字列のみ貼り付けて Log in

> **初回ログイン後はCookieが保存されるため、次回からトークン不要でアクセスできます。**

### トークンを後から確認する

起動後にトークンを忘れた場合は以下で確認できます:
...
> **注意（セキュリティ）**
> - `--no-browser` を必ず付けること（コンテナ内でブラウザを起動しようとするため）
> - `--NotebookApp.token=''` や `--IdentityProvider.token=''` でトークンを無効化しないこと。トークン認証が唯一の防衛線です。
> - ポート 8888 は `127.0.0.1` にバインドされているため LAN 上の他デバイスからは接続できませんが、同一 Windows ホスト上の他ユーザーはアクセス可能な点に留意してください。
