...
- **docker-compose.yml**: ネットワーク名 `claude-network` → `internet-exit` に変更
- **cap_add**: `CHOWN`, `SETUID`, `SETGID`, `DAC_OVERRIDE` を追加（一部ツール互換性）
- **pids_limit**: `deploy.resources.limits.pids` に統合（Compose V2 準拠）
- **ulimits.nofile**: `soft: 1024` → `soft: 4096` に変更（fd 上限を統一）
